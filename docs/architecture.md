...
Store (src/ui/store.ts)       负责订阅、revision bump、存档节流
                              ...
                              因此 UI 可以直接调用 GameSession 暴露的全局指令，
                              并通过 session.focused 调用当前聚焦角色指令
...
新增游戏指令时，优先定义在 `GameSession` 或 `CharacterController` 契约上；Store 继续直接暴露 `GameSession`，而 UI 面向当前角色时默认走 `session.focused`，避免在各面板里重复 `getFocusedCharacter()`。
...
| 基础设施 | — | `flags` | `GameState.flags` 通用键值存储，用于轻量状态标记 | [flags](./modules/flags.md) |
...
| 经济 | `economy/` | `economy` | 通用开销（CostDef）与奖励（RewardBundle）类型、操作与多人分配 | [economy](./modules/economy.md) |
...
| 成长 | `growth/unlock/` | `unlock` | 功能/地点解锁开关，底层复用 flags | [unlock](./modules/unlock.md) |
| 对话 | `dialogue/` | `dialogue` | NPC 对话脚本执行、条件分支、副作用 action | [dialogue](./modules/dialogue.md) |
