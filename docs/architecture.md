...
| 行为 | `effect` `ability` | 通用行为表达与效果结算 | [effect-ability](./modules/effect-ability.md) |
| 战斗 | `combat` `intent` | 战斗状态、自动决策、行动调度与胜负推进 | [combat](./modules/combat.md) |
| 场景 | `stage` `activity` | 场景中的 actor 生命周期与玩家活动 | [stage-activity](./modules/stage-activity.md) |
...
新游戏初始化完全由内容配置驱动：起始英雄与初始 stage 都写在 `ContentDb.starting` 中。未配置时直接抛出异常，不做兜底。
