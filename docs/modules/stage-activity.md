...
2. `cc.startFight(combatZoneId)` — 创建 StageSession 写入 `state.stages`，设 `hero.stageId`，刷首波，创建 CombatActivity（`startPartyCombat` 的单角色便捷入口）
...
多人组队战斗由 `session.startPartyCombat(combatZoneId, partyCharIds)` 直接调用。
...
CombatActivity 已升级为 **WorldActivity**，以 `stageId` 为键，不再绑定单个角色。
...
- 每次 phase 切换后，`syncCombatToHeroes` 把状态同步回所有队员的 `hero.activity`
- `hero.activity` 遵循单一写入者约束：只有 activity 自己会写这个字段
- 首次启动时，`onStart` 会重置战斗前所有队员的 HP / MP，并清空 effects 与 cooldowns
- 从存档恢复时走继续执行路径，跳过这些只应在首次启动时发生的副作用
- **击杀奖励**（XP、货币）按存活队员人数平均分摊，每人拿到整除后的份额
- **波次奖励**：物品一次随机发给某个存活队员；货币在存活队员间平均分摊
- 波次奖励只在 `players_won` 时发放；如果全灭，该波失败且不发波次奖励
- 战斗结束后，如果任意队员 HP 比例低于 combat zone 配置的阈值 `recoverBelowHpFactor`，会先进入 `recovering` 把所有队员血回满，再继续等待下一波
- 战斗结束时会发出 `waveResolved` 事件，供 UI 或其他系统订阅
- session 层以 `Map<stageId, CombatActivity>` 管理战斗活动，与 DungeonActivity 管理方式一致
...
- CombatActivity 的持久化数据包含 `stageId`、`partyCharIds`、`phase`、`currentBattleId`、`lastTransitionTick`
