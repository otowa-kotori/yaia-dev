...
import { computeWorldModifiers, upgradeCost } from "../../../src/core/growth/worldrecord";
import {
  rebuildCharacterDerived,
  getAttr,
} from "../../../src/core/entity/actor";
import { applyEffect } from "../../../src/core/behavior/effect";
...
describe("computeWorldModifiers", () => {
  ...
  test("level N stacks N copies", () => {
    ...
    expect(total).toBe(9); // 3Ã—3
  });

  test("multi-stat upgrade produces N Ã— perLevel entries", () => {
    const record: WorldRecord = { upgrades: { [multiStatUpgradeDef.id]: 2 } };
    const content = makeContentWithUpgrades();
    const mods = computeWorldModifiers(record, content);
    // 2 stats Ã— 2 levels = 4 entries
